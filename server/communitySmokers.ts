...
const STRING_FIELDS = new Set(['name', 'builderName', 'smokerType', 'fuelType', 'metalGauge', 'draftType', 'brand', 'model', 'category', 'insulationType', 'thermalEfficiencyRating', 'controllerType', 'notes']);
const NUMBER_FIELDS = new Set(['chamberVolumeSqIn', 'hopperCapacityLbs', 'baselineBurnRateLbsHr', 'factoryBaselineBurnRateLbsHr', 'factoryHighHeatBurnRateLbsHr', 'bowlCapacityLbs', 'cookingAreaSqIn']);
...
communitySmokersRouter.post('/contribute', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  const title = kind === 'manufacturer' ? [specs.brand, specs.model].filter(Boolean).join(' ') : cleanText(specs.name);
  ...
  const ref = await adminDb.collection('verifiedKnowledge').add({
    type: 'smoker', title,
    claims: Object.entries(specs).filter(([field]) => field !== 'notes').map(([field, value]) => `Community submission states ${field}: ${String(value)}`),
    structuredSpecs,
    provenanceClass: 'USER_ENTERED',
    verificationScope: 'community_submission_pending',
    communityKind: kind,
    source: { url: null, type: 'user_observation', publisher: null, retrievedAt: submittedAt, harvested: false },
    status: 'pending_review', submittedBy: req.user!.uid, submittedAt: FieldValue.serverTimestamp(),
