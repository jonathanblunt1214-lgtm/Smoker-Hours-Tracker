...
export type MeatHarvestCandidate = { type:'meat'; title:string; publisher:string; sourceUrl:string; sourceType:'government'|'culinary_education'|'university_extension'|'approved_primary'; claims:MeatClaim[]; verificationState:'candidate_review_required'; harvestedAt:string };
...
export async function harvestMeatKnowledge(input:MeatHarvestInput):Promise<MeatHarvestCandidate>{
  ...
  if(!sourcePolicy(host(sourceUrl)))throw new Error('Source is not on the approved meat-knowledge allowlist.');
  ...
  return{type:'meat',title:title(html)||host(finalUrl),publisher:host(finalUrl),sourceUrl:finalUrl,sourceType:policy.sourceType,claims,verificationState:'candidate_review_required',harvestedAt:new Date().toISOString()};
}
