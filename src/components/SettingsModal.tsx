...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const defaultTab: Tab = initialTab === 'smokers' ? 'equipment' : initialTab === 'alerts' ? 'notifications' : initialTab === 'cloud' ? 'sync' : initialTab === 'data' ? 'sync' : initialTab;
      ...
      <div className="mx-auto flex min-h-full max-w-5xl flex-col overflow-hidden bg-[#111] sm:min-h-0 sm:rounded-2xl sm:border sm:border-zinc-800 sm:shadow-2xl md:flex-row">
        <aside className="border-b border-zinc-800 bg-zinc-950/80 p-4 md:w-60 md:border-b-0 md:border-r">
          <div className="mb-4 flex items-center gap-3 px-2">
            ...
            <div>
              ...
              <div className="text-xs text-zinc-500">SmokeStack</div>
            </div>
          </div>
          <nav className="flex gap-2 overflow-x-auto pb-1 md:block md:space-y-1 md:overflow-visible">
            {navItems.map((item) => (
              <button
                ...
                className={`flex shrink-0 items-center gap-2 rounded-xl px-3 py-2.5 text-sm transition md:w-full ${tab === item.id ? 'bg-orange-500/15 text-orange-300' : 'text-zinc-400 hover:bg-zinc-900 hover:text-zinc-100'}`}
              >
        ...
        <section className="min-w-0 flex-1">
          <header className="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-800 bg-[#111]/95 px-5 py-4 backdrop-blur">
            <div>
              <h2 className="text-lg font-semibold text-white">{navItems.find((x) => x.id === tab)?.label}</h2>
              <p className="mt-0.5 text-xs text-zinc-500">Production settings use real account and integration state.</p>
            </div>
          ...
          <div className="space-y-5 p-5 sm:p-6">
            ...
            {tab === 'sync' && (
              <>
                ...
                <Card title="Google Drive backup">
                  ...
                  <Row title="Automatic Drive backup" description="Runs only when a valid Drive authorization exists; it does not change account-sync authority." control={<Toggle checked={autoSyncDrive} onChange={onToggleAutoSync} label="Automatic Drive backup" />} />
                </Card>
            ...
            {tab === 'privacy' && (
              <Card title="Privacy & data use">
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs leading-5 text-zinc-500">
                  Cook photos, location/weather, recipes, equipment details, temperature curves and other granular categories remain OFF unless a future consent workflow explicitly enables each category. Personalization for your own CharGPT is separate from sharing.
                </div>
              </Card>
            )}
